...
export default function CreateAccountModal({ isOpen, onClose, onCreateSuccess }: CreateAccountModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await accountsAPI.register({ username, password });
      ...
      toast.error("Error creando la cuenta. Intenta otro nombre de usuario", { 
        position: "top-center" 
      });
    } finally {
      setLoading(false);
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-xl p-6 w-full max-w-md relative">
        ...
        <h2 className="text-2xl font-semibold text-amber-900 mb-6">Crear Nueva Cuenta</h2>
